...
> Last updated 2026-04-23. This section reflects actual deployed state; the rest of the document is the original design reference.
...
| **Default model** | Gemini Flash via Kilo | Restored to **`litellm/kilo-sonnet`** on 2026-04-25 after the heartbeat-cost fix (isolatedSession+lightContext+1h+activeHours, applied 2026-04-23) verifiably brought daily Kilo spend down. Compaction stays pinned to Flash; `vehicle-reminders` cron stays pinned to Flash. LiteLLM fallback chain extended: `kilo-sonnet → gemini-3-pro-preview → gemini-3-flash-preview` so a Kilo/Anthropic blip falls back instead of 500-ing. Earlier history: Sonnet was reverted to Flash on 2026-04-23 after spend hit $30–46/day (heartbeat re-sending 150k-token Sonnet prompts every ~14 min on a session live since 2026-04-01); root cause was the heartbeat config, not Sonnet itself. |
