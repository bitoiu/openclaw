...
> Last updated 2026-06-29. This section reflects actual deployed state; the rest of the document is the original design reference.

### 2026-06-29 — Bumped to 2026.6.10 stable (5-release delta from 6.5)

Clean upgrade, no breakage. Delta analysis done first per process (start from deployed 6.5; stable line only — skipped betas/alphas).

- **Upgraded 2026.6.5 → 2026.6.10 stable** (released 24 Jun). Stable releases in the gap: 6.6, 6.8, 6.9, 6.10 (6.7 beta-only). Healthy in ~5s, `[gateway] ready`, default `litellm/sonnet-4-6`, WhatsApp listening (DM + 1 group). Live `openclaw agent` turn returned end-to-end (`upgrade-smoke-ok`).
- **Auth migration was the headline risk — and it held.** 6.6 (`#91740` verify SQLite auth migration before cleanup) and 6.9 (`#93156` import default-agent auth profiles into sqlite, `#93448` allow auth profile sqlite reader) all touch the same per-agent SQLite auth store that silently broke us on the 6.5 bump. This time `openclaw models status` showed `litellm:manual=token:sk...34` intact post-upgrade — our `paste-token` profile survived. **No re-auth needed.** Still the #1 thing to verify on every future bump.
- **WhatsApp / Baileys patch survived.** 6.6 removed bundled channel contract fallbacks (`#91871`) and 6.9 loads external channel plugins at gateway startup — but the `@openclaw/whatsapp` plugin did **not** update (same hashed dir `openclaw-whatsapp-290d7f7427`), so the in-place `passive:false` patch stayed intact (2/2 sites). No re-patch needed; re-run `scripts/openclaw-wa-passive-patch.sh` only if a future bump changes the hash/baileys version.
- **Net wins picked up:** cron auth-failure isolation so a failing cron job won't poison the shared auth cooldown (`#91187`); OpenAI embedding batches split before 431 (our `memorySearch` uses `text-embedding-3-small`, `#92650`); dreaming ingestion audit after JSON migration (`#92020`); doctor no longer false-warns on the migrated cron store (`#92690`).
- **No-impact items confirmed:** WAL-on-NFS fix (`#93454`) N/A — `config/openclaw` is local ext4, not the NAS NFS mount; OpenRouter OAuth onboarding and `openrouter/`-prefix normalization don't touch us (we auth LiteLLM via API key, bare model ids); compaction default-timeout→180s fine (flash compaction).
- **New advisory (follow-up candidate):** boot now logs `plugins.allow is empty; discovered non-bundled plugins may auto-load: whatsapp`. The 6.9 startup-load behavior means our externalized WhatsApp plugin auto-loads unpinned. Given our security posture, consider setting `plugins.allow` to explicit trusted ids (`whatsapp`, `searxng`, `memory-core`, `litellm`) — not blocking, deferred.
- **Watches still OPEN:** (a) **Baileys `passive:true→false` upstream fix** — still not merged through 6.10; in-place patch remains required. (b) **SearXNG native OpenClaw support** (#43822) — not landed; SearXNG plugin retained.
...
### 2026-06-28 — Power outage → cAdvisor lost Docker labels → 23 false ContainerDown/VpnDown alerts

A power cut rebooted the host. All 25 containers came back healthy (RestartCount 0, all healthchecks green, OpenClaw gateway ready + WhatsApp connected), but Alertmanager fired **22× `ContainerDown` + 1× `VpnDown` (all `severity=critical`, every service "DOWN")** — pure false positives. Root cause was **not** the outage directly: Docker Engine had been upgraded to **29.4.0 (min API 1.40)**, while cAdvisor (`gcr.io/cadvisor/cadvisor:latest` = v0.49.1) defaults its Docker client to **API 1.24**. cAdvisor kept a working connection until the power-cut forced it to restart — on restart its **Docker container factory failed to register** (`client version 1.24 is too old`), so it fell back to the containerd/raw cgroup factories and emitted 152 `container_last_seen` series with **zero `container_label_com_docker_compose_service` labels**. Both alert rules join on that label, so the `or on(service) (0 * up{job="expected"})` branch filled `0` for every expected service → everything looked down.

- **Fix:** added `environment: [DOCKER_API_VERSION=1.40]` to the `cadvisor` service in `docker-compose.yml` (the Docker Go SDK honours this env var), then `docker compose up -d cadvisor` (recreate — `restart` won't apply env changes). Docker factory then registered "successfully"; compose labels returned (50 labelled series), and both alert expressions dropped to **0 firing** in Prometheus and **0 active** in Alertmanager. No rule or container changes needed — the `127.0.0.1:1`/`job=expected` synthetic targets are working as designed.
- **Pre-existing scrape gaps (unrelated, NOT paging):** `job=media-services` (radarr/sonarr/prowlarr/bazarr return JSON/HTML, no `/metrics`), `job=watchtower` (401), `job=gluetun` metrics endpoint (DNS) — these scrape jobs aren't wired to the ContainerDown rule, so they don't alert.
- **Watch:** cAdvisor `:latest` is pinned-by-drift to v0.49.1 and still defaults to API 1.24; any future Docker Engine bump that raises the min API past 1.40 will recur. The env pin holds as long as the daemon keeps 1.40 in its supported range.
